...
2. Let’s implement the ```Retriever``` interface in a non-generic class.
    ...
    In **Book.java**, implement the ```Retriever``` interface in ```Book``` and override ```retrieveData()```. The body of ```retrieveData()``` should return the ```name``` field in ```Book```. In this case, ```Book``` is a ```Retriever``` of ```String```s.
